...
' *************************************************************************
'  Copyright ©1996-2009 Karl E. Peterson
...
Private Declare Function OemToCharBuff Lib "user32" Alias "OemToCharBuffA" (ByVal lpszSrc As String, lpszDst As Any, ByVal cchDstLength As Long) As Long
...
Public Function ReadChar() As Byte   'KeyAscii
   ...
   ' translated from OEM to current system codepage.
   ReadChar = Char
End Function
...
Public Function ReadLine(Optional ByVal Prompt As String = "", Optional NumChars As Long = 0) As String
   ...
   Do
      ...
      If CharsRead Then
         ...
         If Char = 13 Then
            ' All done!  Pop the final LF off queue.
            Call ReadFile(m_StdInput, Char, 1&, CharsRead, ByVal 0&)
            Exit Do
         ElseIf Char = 10 Then
            ' Linefeed - ignore.
...
Public Function ReadPassword(Optional Prompt As String = "", Optional PasswordChar As String = "*") As String
   ...
   If Len(Prompt) Then
      ' Translate from Windows ANSI to OEM codepage.
      ReDim oemBuffer(1 To Len(Prompt)) As Byte
      If CharToOemBuff(Prompt, oemBuffer(1), UBound(oemBuffer)) Then
         'Call WriteConsole(m_StdOutput, ByVal Prompt, Len(Prompt), CharsWritten, ByVal 0&)
         Call WriteConsole(m_StdOutput, oemBuffer(1), UBound(oemBuffer), CharsWritten, ByVal 0&)
      End If
   End If
   ...
   Do
      ...
      Select Case KeyAscii
         ...
         Case Else
            ...
            If Len(PasswordChar) Then
               ReDim oemBuffer(1 To Len(PasswordChar)) As Byte
               If CharToOemBuff(PasswordChar, oemBuffer(1), UBound(oemBuffer)) Then
                  Call WriteConsole(m_StdOutput, oemBuffer(1), UBound(oemBuffer), CharsWritten, ByVal 0&)
               End If
            End If
      End Select
...
Public Function WriteLine(Optional ByVal Text As String = "", Optional ByVal CrLf As Boolean = True, Optional Destination As ConsoleOutputDestinations = conStandardOutput, Optional Alignment As ConsoleWriteAlignments = conAlignNone) As Long
   ...
   If Len(Text) Then
      ' Translate from Windows ANSI to OEM codepage.
      ReDim oemBuffer(1 To Len(Text)) As Byte
      If CharToOemBuff(Text, oemBuffer(1), UBound(oemBuffer)) Then
         'Call WriteFile(hOutput, ByVal Text, Len(Text), BytesWritten, ByVal 0&)
         Call WriteFile(hOutput, oemBuffer(1), UBound(oemBuffer), BytesWritten, ByVal 0&)
...
Public Sub PressAnyKey(Optional ByVal Prompt As String, Optional ByVal CrLf As Boolean = True)
   ...
   ' Scroll if requested; no translation necessary.
   If CrLf Then
      Prompt = vbCrLf
      Call WriteConsole(m_StdOutput, ByVal Prompt, Len(Prompt), CharsWritten, ByVal 0&)
   End If
...
Private Function GetProcessFileName(ByVal ProcessID As Long) As String
   ...
   Else
      ...
      If hProcess Then
         ...
         Call GetModuleFileNameEx(hProcess, 0, Buffer, Len(Buffer))
         Call CloseHandle(hProcess)
      End If
   End If
   
   ' Return results.
   GetProcessFileName = TrimNull(Buffer)
End Function
